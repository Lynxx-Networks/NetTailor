...
#[derive(Deserialize)]
pub struct NettailorCheckResponse {
    nettailor_instance: Option<bool>,
}

pub async fn verify_nettailor_instance(server_name: &str) -> Result<NettailorCheckResponse, anyhow::Error> {
    let url = format!("{}/api/nettailor_check", server_name);
    let response = Request::get(&url).send().await?;

    if response.ok() {
        let check_data: NettailorCheckResponse = response.json().await?;
        if check_data.nettailor_instance.unwrap_or(false) {
            Ok(check_data)
        } else {
            Err(anyhow::Error::msg("Nettailor instance not found"))
        }
    } else {
        Err(anyhow::Error::msg("Failed to verify Nettailor instance"))
    }
}
